...
cascade_face = cv2.CascadeClassifier('def.xml')
...
def detection(grayscale, img):
    ...
    for (x_face, y_face, w_face, h_face) in face:
        ...
        for (x_smile, y_smile, w_smile, h_smile) in smile:
            ...
            smile_percent = (w_smile * h_smile) / (w_face * h_face) * 100
            cv2.putText(img, f"Smile: {smile_percent:.2f}%", (x_face, y_face + h_face + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    return img
